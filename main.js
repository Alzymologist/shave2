...
if(window.addEventListener && window.history) setInterval(function(x) { if(Object.keys(mHelps).length) return;
    var w=document.location.href.split('?')[1]; if(w && !(w.indexOf('win_')&&w.indexOf('win2_')) && window.history.back) window.history.back();
},100);
...
if(typeof(hotkey_default)!='function') hotkey_default=function(){
	...
	setkey('KeyE','',function(e){majax('editor.php',{a:'editform',num:num,comments:(idd('commpresent')?1:0)})},false); // редактор заметки
...
function doclass(cla,f,s,node) {
    var p=getElementsByClass(cla,node?node:document);
    for(var i in p) { if(typeof(p[i])!='undefined' && typeof(p[i].className)!='undefined') f(p[i],s); }
//  var p=(node?node:document).querySelectorAll('.'+cla);
// for(var i=0;i<p.length;i++) f(p[i],s);
}
...
// function cpm(a) {
//    var b=a.slice();
//    for(var i in b) { if(typeof(b[i])=='array' || typeof(b[i])=='object') b[i]=cpm(b[i]); }
//    return b;
// }
...
/*
function printr_c(ev,e){ ev.stopPropagation();
    e.innerHTML=e.querySelector('DIV').innerHTML;
    e.className='';
    e.style.display='inline-table';
    e.className='';
    e.cursor='auto';
    e.style.color='#559';
			    // vl="<div onclick='printr_c(event,this)' class=ll>function<div style='display:none'>"+v+"</div></div>";
}
*/
...
clean=function(id,animtrue) {
    ...
    if(typeof(mHelps[id])!='undefined' && idd(id).className!='newin') { // окно было
	...
	var clen=function(){var s=idd(id); if(!s) return;
		if(idd(id).className=='newin') {
		    s.previousSibling.style.display='block';
		    rootElement=document.body.lastChild.previousSibling;
		    document.documentElement.scrollTop=rootElement.getAttribute('scrollTop');
		    document.documentElement.scrollLeft=rootElement.getAttribute('scrollLeft');
		}
		s.parentNode.removeChild(s);
	};

        if(typeof(idd(id).onanimationend)!='object' || in_array(id,['tenek','ajaxgif'])) { zakryl(id); setTimeout(function(){clen()},40); }
        else anim(idd(id),'zoomOut',clen,animtrue);

    } else if(typeof(idrename)!='undefined'&&typeof(idrename[id])!='undefined') { clean(idrename[id]); }
        zakryl('tip');

//    clhis=function(iid) { if(window.history && window.history.back && window.history.state && i!='salert' && window.history.state.win === iid) {
// if(admin) alert('History clean: '+iid+'document.location.href1: '+document.location.href.split('?')[1]);
// window.history.back();
// if(admin) alert('History clean: '+iid+'document.location.href2: '+document.location.href.split('?')[1]);
// }
// };
//    setTimeout("clhis('"+id+"')",250);
//    setTimeout("clhis('"+id+"')",500);
...
function bukadump() { // отладочник
    ...
    s+='<hr>';
    zabil('bukadump',vzyal('bukadump')+'<hr>'+s);
}
...
if(!user_opt('nowin')) {
    ...
    if(!idd('mainbody')) { document.body.innerHTML="<div id='mainbody'>"+vzyal(document.body)+"</div>"; init_tip(); }
    if(!rootElement) rootElement=idd('mainbody');

    if(!addEventListenerSet && window.addEventListener && window.history) {
	    addEventListenerSet=1;
	    window.addEventListener('popstate', function(e){ if(!e) return;

	    if(e.state && e.state.win && idd(e.state.win) ) {
/*
		if(unic==4) alert('Back from: '+e.state.win+"\
 window.history.back="+(window.history.back?1:0)+"\
 idd(e.state.win)="+( idd(e.state.win) ? 1:0 )+"\
 idd(e.state.win).className="+idd(e.state.win).className
);
*/
		if(window.history.back /*&& idd(e.state.win) && idd(e.state.win).className!='111newin'*/ ) {
		    // if(unic==4) alert('Clean: '+e.state.win);
		    clean(idd(e.state.win));
		    // if(unic==4) alert('Cleaned: '+e.state.win);
		    // window.history.back();
		}
	    } // else {		if(unic==4) alert('False e.state.win');	    }

/*
	    if(!e.state || !idd(e.state.win) ) {
		if(window.history.back && e.state && e.state.win && idd(e.state.win).className=='newin' ) {
		//	if(unic==4) alert('Back from: '+e.state.win);
		window.history.back();
	        }
	    } else {
	//	if(unic==4) alert('Clean: '+e.state.win);
		if(e.state.win) clean(e.state.win);
	    }
*/
	...
	newdiv("<div class='winret' onclick='if(window.history.back) window.history.back(); else clean(this.parentNode)' style='font-size:50px;index:30;position:absolute;top:5px;left:5px;cursor:pointer;width:100%;text-align:left;'>&#9664;</div>\
<div style='margin: 80px 10px 0 10px; text-align:left' class='"+cls+"' id='"+id+"_body'>"+s+"</div>",{id:id,cls:'newin'},document.body);
	init_tip(idd(id));
	rootElement=idd(id); // document.body.lastChild;
	if(window.history && window.history.pushState) {
		window.history.pushState({win:id},'?newin2_'+id,'?newin2_'+id);
		window.history.pushState({win:id},'?newin_'+id,'?newin_'+id);
	}

    } else { if(idd(id+'_body')) zabil(id+'_body',s); }
...
function ti(id,tmpl) { // заменить выделенное на tmpl, в котором {select} заменится на выделенное
    var e=idd(id),v=e.value,ss=e.selectionStart,es=e.selectionEnd;
...
function init_tip(w) { w=w||document; if(!idd('tip')) {
...
function salert(l,t) {
    var p=idd('salert');
    if(p){ p=getElementsByClass('textbody',p)[0]; if(p) { p.innerHTML=p.innerHTML+'<hr>'+l; return false; } }
...
function anim(e,i,fn,animeffect) {
    ...
    if( typeof(idd(e).onanimationend) != 'object' ) {
...
NFC={
	...
	salert("Увы, этот браузер не поддерживает чтение NFC-карт",4000);
        ...
        NFC.ndef.onreading = (ev) => { if(!idd('NFC.tenek')) return;
	    NFC.off();
