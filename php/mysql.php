...
if(function_exists('mysqli_report')) mysqli_report(MYSQLI_REPORT_OFF); // о как заебали эти нововведения PHP
...
// $GLOBALS['msq_strict']='STRICT_TRANS_TABLES'; // эту строчку надо вписать в config.php если mysql ругается при установке на default

    if(isset($GLOBALS['msq_strict']) && false!==strpos($o=ms("SELECT @@sql_mode;","_l"),$GLOBALS['msq_strict']))
	    msq("SET sql_mode = '".implode(',',array_diff(explode(',',$o),array($GLOBALS['msq_strict'])))."'");
...
function msq_add($tb,$ara,$ignore=false) {
        $a=$b=''; foreach($ara as $n=>$m) { $a.="`$n`,"; $b.="'$m',"; } $a=trim($a,','); $b=trim($b,',');
        $s = "INSERT".($ignore?" IGNORE":'')." INTO $tb ($a) VALUES ($b)";
        return msq($s);
}

function msq_add1($tb,$ara) {
        $a=$b=''; foreach($ara as $n=>$m) { $a.="`$n`,"; $b.="$m,"; } $a=trim($a,','); $b=trim($b,',');
        $s = "INSERT IGNORE INTO $tb ($a) VALUES ($b)";
        return msq($s);
}
...
function msRavno($n,$v) { // йобаные патчи для сраного MySQL который не понимает численных переменных в кавычках
	$dar=array('i','n','id','num','acn','unic','DateID','Parent','Time','IPN','commentID','view_counter','DateUpdate','DateDatetime','DateDate');
	return "`".e($n)."`=".(in_array($n,$dar) ? intval($v) : "'".e($v)."'");
}

/*
function msq_add_update0($tb,$ara,$u='id') {
        $a=$b=$c''; foreach($ara as $n=>$m) { $a.="`$n`,"; $b.="'$m',"; $c.="`$n`='$m',"; }
	$a=trim($a,','); $b=trim($b,','); $c=trim($c,',');

	if(!stristr($u,'WHERE ')) { $keys=explode(' ',$u);
		$u=array(); foreach($keys as $k) { if($k=='ANDC') break; $u[]=msRavno($k,$ara[$k]); }
		$u="WHERE ".implode(' AND ',$u).($k=='ANDC'?ANDC():'');
	}

        $s = "INSERT INTO $tb ($a) VALUES ($b) ON DUPLICATE KEY UPDATE $c $u";
        return msq($s);
}
*/
...
function msq_add_update($tb,$ara,$u='id') {
        ...
        else { if(isset($keys) && sizeof($keys)) { foreach($keys as $k) unset($ara[$k]); } $s=msq_update($tb,$ara,$u); }
        return $s;
}
...
function msq_del($tb,$ara,$u='') {
	$a=''; foreach($ara as $n=>$m) $a.=msRavno($n,$m)." AND "; $a=substr($a,0,-5);
...
function msq($s) { global $msqe;
	...
	if(time()-$GLOBALS['starttime']>15) {
		if(function_exists('logi')) logi('starttime.log',"\nerror: ".$GLOBALS['MYPAGE']);
...
if($t>0.1 && function_exists('logi')) logi('msq_time.log',$t."  ".$s."\n\n");

	return($sql);
}

function msq_pole($tb,$pole,$schema='') { // проверить, существует ли такое поле в таблице $tb
    return ms("SELECT COLUMN_TYPE FROM information_schema.columns WHERE table_schema='".e($schema==''?$GLOBALS['msq_basa']:$schema)."' AND table_name='".e($tb)."' AND COLUMN_NAME='".e($pole)."'",'_l',0);
// if(!msq_table($tb)) return false;      $pp=ms("SHOW COLUMNS FROM ".$tb."","_a",0); foreach($pp as $p) if($p['Field']==$pole) return $p['Type']; return false;
}

function msq_table($table,$schema='') { // проверить, существует ли такая таблица
        if(preg_match("/^`([^`]+)`\.`([^`]+)`$/s",$table,$m)) list(,$schema,$table)=$m;
        else if($schema=='') $schema=$GLOBALS['msq_basa'];
        return (bool)ms("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='".e($schema)."' AND table_name='".e($table)."'",'_l',0);
}

function msq_index($tb,$index,$schema='') { // проверить, существует ли такой индекс (результат: PRIMARY KEY - 1, KEY - true)
	return (($p=ms("SELECT * FROM INFORMATION_SCHEMA.STATISTICS WHERE table_schema='".e($schema==''?$GLOBALS['msq_basa']:$schema)."' AND table_name='".e($table)."'
AND INDEX_NAME='".e($index)."'",'_1',0)) ? ($p['INDEX_NAME']=='PRIMARY'?1:true) : false );
/*
...
//	if($p['Column_name']==$index && $p['Non_unique']=='1') return true; // [Seq_in_index] => 1
	if($p['Column_name']==$index || $p['Key_name']==$index) return ($p['Key_name']=='PRIMARY'?1:true);
	return false;
*/
}
...
if(function_exists('mysqli_connect')) {
	...
	elseif($mode == '_a1') { $s=array(); while($p=mysqli_fetch_assoc($sql)) $s[]=current($p); }
	...
	if($mode == '_a1') { $s=array(); while($p = mysql_fetch_assoc($sql)) $s[]=current($p); }
...
function cache_set_raw($k,$v,$e) { global $memcache; if(!$memcache) return false; return memcache_set($memcache,$k,$v,MEMCACHE_COMPRESSED,$e); }
